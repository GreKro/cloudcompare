...
#' }
sor <- function(file, neighbors=50, sigma=1, output_dir='C:\\output.las',
  ...
  if(class(neighbors) != 'numeric' ){
    stop(paste('Wrong neighbours is a ', class(file),', must be a numeric'))

  }

  if(class(sigma) != 'numeric' ){
    stop(paste('Wrong sigma is a ', class(file),', must be a numeric'))
  ...
  s5= paste('-SOR',neighbors,sigma, sep=' ')
